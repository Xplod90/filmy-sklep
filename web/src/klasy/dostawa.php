...
class Dostawa { 
  ...
  */
  function save($db) {
    ...
    $query->execute(array($this->_adres, $this->_imie, $this->_nazwisko));
    // Pobieranie id 
  ...
  function get($db, $id) {
    ...
    $query = $db->prepare("SELECT `id`,`adres`, `imie`, `nazwisko` FROM `dostawa` WHERE `id`=? LIMIT 1;");
    ...
    foreach ($query->fetchAll() as $row_id => $row_data) {
      ...
      $this->_nazwisko = $row_data["nazwisko"];
    }
  }
}
